...
wbi_config[
    "USER_AGENT"
] = "MyWikibaseBot/1.0 (https://www.wikidata.org/wiki/User:Ada-2023-project-adaptateurs)"


def get_gender_wiki(wikiID, job):
    dictory = {"director": "P57", "producer": "P162", "writer": "P58"}
    key = dictory[job]
    wiki_wiki = wikipediaapi.Wikipedia("ada (ada@epfl.ch)", "en")
    wbi = WikibaseIntegrator()
    page_id = wiki_wiki.page(wikiID)
    page_name = requests.get(
        f"http://en.wikipedia.org/w/api.php?action=query&pageids={wikiID}&format=json"
    ).json()["query"]["pages"][
        wikiID
    ]  # ['title']
    qid_movie = requests.get(
        f"https://en.wikipedia.org/w/api.php?action=query&prop=pageprops&titles={page_name}&format=json"
    ).json()["query"]["pages"][wikiID]["pageprops"]["wikibase_item"]
    ...
    for element in qid_directors:
        genders.append(
            wbi.item.get(entity_id=element).get_json()["claims"]["P21"][0]["mainsnak"][
                "datavalue"
            ]["value"]["id"]
        )
    return genders
...
def get_gender_imdb(imdb_id, job, provided_name):
    try:
        ...
        key = dictory[job]
        wbi = WikibaseIntegrator()
        query = wbi_helpers.execute_sparql_query(
            "SELECT ?film ?filmLabel WHERE {?film wdt:P345 "
            + imdb_id
        ...
        for element in qid_directors:
            ...
            try:
                ...
                first_name = wbi.item.get(entity_id=first_name_id).get_json()["labels"][
                    "en"
                ]["value"]
                last_name = wbi.item.get(entity_id=last_name_id).get_json()["labels"][
                    "en"
                ]["value"]
...
def get_IMDB_ID(wikiID):
    ...
    page_name = requests.get(
        f"http://en.wikipedia.org/w/api.php?action=query&pageids={wikiID}&format=json"
    ).json()["query"]["pages"][wikiID]["title"]
    qid_movie = requests.get(
        f"https://en.wikipedia.org/w/api.php?action=query&prop=pageprops&titles={page_name}&format=json"
    ).json()["query"]["pages"][wikiID]["pageprops"]["wikibase_item"]
    movie_item = wbi.item.get(entity_id=qid_movie)
    IMDB_ID = movie_item.get_json()["claims"]["P345"][0]["mainsnak"]["datavalue"][
        "value"
    ]
    return IMDB_ID
